...
public class GpuShadows implements ApplicationListener {
  ...
  public static void main(String[] args) {
		new Lwjgl3Application(new GpuShadows(),getDefaultConfiguration());
	}


	private static Lwjgl3ApplicationConfiguration getDefaultConfiguration() {
		Lwjgl3ApplicationConfiguration configuration = new Lwjgl3ApplicationConfiguration();
		configuration.setTitle("tests");
	...
	public void create() {
		...
		final String VERT_SRC = Gdx.files.internal("assets/shaders/pass.vert").readString();
		
		// renders occluders to 1D shadow map
		shadowMapShader = createShader(VERT_SRC, Gdx.files.internal("assets/shaders/shadowMap.frag").readString());
		// samples 1D shadow map to create the blurred soft shadow
		shadowRenderShader = createShader(VERT_SRC, Gdx.files.internal("assets/shaders/shadowRender.frag").readString());
		
		//the occluders
		casterSprites = new Texture("assets/cat4.png");
		//the light sprite
