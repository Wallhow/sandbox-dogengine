...
class SVelocity : IteratingSystem(Family.all(CTransforms::class.java, CVelocity::class.java).get()) {
    ...
    init {
        ...
        if (Kernel.getInjector().getInstance(dogengine.es.redkin.physicsengine2d.world.World::class.java) != null)
            ppu = Kernel.getInjector().getInstance(dogengine.es.redkin.physicsengine2d.world.World::class.java).PPU
    }
