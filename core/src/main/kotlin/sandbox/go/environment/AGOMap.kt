...
abstract class AGOMap(name: String) : AGameObject(name) {
    ...
    protected fun createCHealth(maxHealth: Float,count: Int = 1,drop: ObjectList? = dropID, beforeDeadFunc: (() -> Unit)? = null) {
        create<CHealth> {
            health = maxHealth
            beforeDead = beforeDeadFunc
            if(drop!=null)
                this.dropType = CHealth.DropAndCount(count,dropType = drop )
        }
    }
