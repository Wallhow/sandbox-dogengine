...
import sandbox.dev.ecs.world.MessagesType
...
class SExtraction : IteratingSystem(Family.all(CHealth::class.java, CExtraction::class.java).get()) {
    private val messenger = Kernel.getInjector()[Messenger::class.java]
    init {
    ...
    override fun processEntity(entity: Entity, deltaTime: Float) {
        ...
        if (health.health < 0) {
            ...
            for (x1 in 0..(s.width/32).toInt()) {
                ...
                messenger.sendMessage(MessagesType.EXTRACTION,Vector2Int(x+x1,y))//Отправляем сообщение
