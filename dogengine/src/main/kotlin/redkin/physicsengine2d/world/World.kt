package dogengine.redkin.physicsengine2d.world
...
import dogengine.redkin.physicsengine2d.utils.Functions
import dogengine.redkin.physicsengine2d.utils.Functions.Companion.rectGetVertices
...
class World(gravity: Float, val PPU: Float = 1f) {
    ...
    fun drawDebug(cam: OrthographicCamera, shapeRenderer:ShapeRenderer) { // Draw Objects
    ...
    fun drawDebug(cam: OrthographicCamera, shapeRenderer:ShapeDrawer) { // Draw Objects
        shapeRenderer.setColor(0f, 1f, 0f, 1f)
        if (showDebug) {
            shapeRenderer.line(lineCheckerCenter.getP1(), lineCheckerCenter.getP2())
            shapeRenderer.line(lineCheckerLeft.getP1(), lineCheckerLeft.getP2())
        ...
        for (polyline in polylines) {
            shapeRenderer.polygon(polyline.transformedVertices)
        }
        for (rectangle in rectangles) {
            shapeRenderer.rectangle(rectangle.x*PPU, rectangle.y*PPU, rectangle.width*PPU, rectangle.height*PPU)
        }
        ...
        if (drawSensors) {
            ...
            for (rectangle in rectangles) {
                for (sensor in rectangle.sensors) {
                    shapeRenderer.rectangle(sensor.x, sensor.y, sensor.width, sensor.height)
                }
            }
        }
    }
